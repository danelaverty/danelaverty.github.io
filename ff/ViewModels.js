...
(function(ChakraApp) {
    ...
    if (circleModel.colors && Array.isArray(circleModel.colors) && circleModel.colors.length > 0) {
    this.colors = circleModel.colors.slice(); // Copy the array
  } else if (circleModel.color) {
    this.colors = [circleModel.color]; // Convert single color to array
  } else {
    this.colors = ['#C0C0C0']; // Default
  }
    ...
    this.text = circleModel.text || null;
  ...
  ChakraApp.CircleViewModel.prototype.hasMultipleColors = function() {
  
  var viewModelColors = this.colors && Array.isArray(this.colors) ? this.colors : [];
  var modelColors = this.model && this.model.colors && Array.isArray(this.model.colors) ? this.model.colors : [];
  
  var result = viewModelColors.length > 1 || modelColors.length > 1;
  
  return result;
};

  ChakraApp.CircleViewModel.prototype.getColors = function() {
    return this.colors ? this.colors.slice() : [this.color];
  };
  ...
  // Update view model properties from model
   ChakraApp.CircleViewModel.prototype._updateFromModel = function() {
    ...
    var previousColors = this.colors ? this.colors.slice() : [];
    ...
    this.colors = this.model.colors ? this.model.colors.slice() : [this.model.color]; // NEW: Update colors array
    ...
    this.text = this.model.text || null;
    ...
    var colorsChanged = JSON.stringify(previousColors) !== JSON.stringify(this.colors);
    ...
    this.notify({ 
      type: 'update',
      colorsChanged: colorsChanged // NEW: Indicate if colors changed
    });
  };
  ...
  ChakraApp.CircleViewModel.prototype.updateCharacteristic = function(key, value) {
    ...
    if (key === 'color') {
      update.color = value;
      update.colors = [value]; // NEW: Reset to single color when setting via characteristic
    } else {
  ...
  ChakraApp.CircleViewModel.prototype.updateColor = function(color, crystal) {
    ChakraApp.appState.updateCircle(this.id, { 
      color: color, 
      colors: [color], // Reset to single color
      crystal: crystal 
    });
  };

  ChakraApp.CircleViewModel.prototype.addColor = function(color, crystal) {
    var circle = ChakraApp.appState.getCircle(this.id);
    if (circle) {
      circle.addColor(color, crystal);
    }
  };
  
  // NEW: Remove color from existing colors
  ChakraApp.CircleViewModel.prototype.removeColor = function(color) {
    var circle = ChakraApp.appState.getCircle(this.id);
    if (circle) {
      circle.removeColor(color);
    }
  };
  
  // NEW: Set multiple colors
  ChakraApp.CircleViewModel.prototype.setColors = function(colors, crystal) {
    ChakraApp.appState.updateCircle(this.id, { 
      colors: colors.slice(), // Copy array
      color: colors[0] || '#C0C0C0', // First color becomes primary
      crystal: crystal 
    });
  };
